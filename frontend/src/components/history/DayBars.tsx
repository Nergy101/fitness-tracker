...
export default function DayBars({
  ...
  const days = useMemo(() => {
    ...
    const now = new Date();
    const empty: DayCounts = { workout: 0, run: 0, walk: 0, boxing: 0, cycling: 0 };
  ...
  const max = Math.max(1, ...days.items.map((d) => d.counts.workout + d.counts.run + d.counts.walk + d.counts.boxing + d.counts.cycling));
